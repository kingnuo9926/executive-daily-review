...
class Store {
  ...
  createSession({ userId, date, energyScore, overallMood }) {
    ...
    const session = {
      ...
      status: 'active',
      segments: [],
      state: null,             // 对话进度快照，用于刷新后恢复（见 saveSessionState）
  ...
  /**
   * 保存对话进度快照，供刷新页面后恢复（stage / 本段轮次 / 消息历史 / 标签 / 行动项）。
   * 已结束（done）的会话不再接受写入。
   */
  saveSessionState(sessionId, state) {
    ...
    s.state = {
      ...
      stageUserTurns: Number(state.stageUserTurns) || 0,
      messages: Array.isArray(state.messages) ? state.messages : [],
      stageTranscripts: state.stageTranscripts || {},
      stageTags: state.stageTags || {},
  ...
  finishSession(sessionId, { energyScore, overallMood, segments } = {}) {
    const s = this.getSession(sessionId);
    if (!s) return null;
    if (energyScore != null) s.energyScore = energyScore;
    if (overallMood) s.overallMood = overallMood;
    // 幂等：以本次提交的四段整体覆盖，避免重复提交导致 segment 翻倍
    if (Array.isArray(segments)) {
      s.segments = segments.map((seg, i) => ({
        id: 'seg_' + s.id + '_' + (seg.stage || i + 1),
        stage: seg.stage,
    ...
    s.status = 'done';
    if (s.state) s.state.finished = true;
