...
| 语音形态 | 已支持浏览器语音输入与回复播报（Web Speech API）；全双工打断为下一阶段 |
...
```bash
npm test              # 全部（冒烟 29 项 + LLM 路径 mock 24 项）
...
## 路线图（§6.2）
1. ✅ MVP：四段式 + 轮流对话 + 周报
2. ✅ 语音第一阶：浏览器语音输入（Web Speech API）+ 教练回复语音播报（TTS），播报期间自动暂停拾音防回声
3. 语音打磨：全双工打断 / 停顿 / 自然度（流式 ASR + 流式 TTS 管线）
4. 聚合深化：月报、季报、主题画像
5. 智能追问：跨日模式识别、个性化教练
6. 多端发布：iOS + Android、团队/教练后台（可选）

## 语音对话（级联管线）
架构为 **ASR → 文本 LLM → TTS**（不需要多模态大模型）：

- **语音输入（已完成）**：浏览器 Web Speech API（`SpeechRecognition`，中文），Edge/Chrome 内置免配置；识别结果实时回显，完整句自动发送。不支持的浏览器自动禁用麦克风按钮。
- **语音播报（已完成）**：浏览器 `speechSynthesis` 合成中文语音播报教练回复，设置页可开关（存 `prefs.voiceReply`）。
- **回声防护**：请求与播报期间暂停拾音，播报结束后自动恢复监听。
- **第 2 阶段（待做）**：换服务端流式 ASR（讯飞/Paraformer）与流式 TTS（火山/Azure），实现打断（barge-in）与 VAD，浏览器 API 延迟与打断能力均不足。
...
3. **演示模式为规则式教练**：无密钥时按「每段两轮 + 过渡语」推进，主题标签走关键词词典匹配；真实追问深度需接入 LLM 后体现。已与智谱（glm-4.5-air）实机联调通过，换其他厂商填 Key 后点「测试连接」即可确认。
4. **语音打断未实现**：语音输入与播报已可用，但教练播报时用户无法插话打断（需第 3 阶段流式管线 + VAD）；Web Speech API 在部分网络环境下（如 Chrome 的 Google 服务）可能不可用，建议使用 Edge。
5. **周报仅本周**：未做历史周切换与月/季报（§5.3 路线图第 4 阶段）。
